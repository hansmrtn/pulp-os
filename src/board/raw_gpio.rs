...
//!
//! Only implements `embedded_hal::digital::OutputPin` — enough for SPI CS.
...
///
/// Implements `OutputPin` + `ErrorType` from embedded-hal so it can
/// be used as a chip-select pin with `RefCellDevice` / `ExclusiveDevice`.
...
impl RawOutputPin {
    ...
    pub unsafe fn new(pin: u8) -> Self {
        ...
        // Read-modify-write to preserve reserved bits, set function to GPIO.
        // Bits [2:0] = MCU_SEL = 1 (GPIO function)
        unsafe {
            let val = mux_reg.read_volatile();
            let val = (val & !0b111) | 1;
            mux_reg.write_volatile(val);

            // Configure GPIO matrix: enable output for this pin
