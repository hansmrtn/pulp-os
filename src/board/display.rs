    ...
    BUSY: InputPin,
{
    ...
    /// Wait for the display controller to finish an operation.
    ///
    /// The BUSY pin is HIGH while the controller is working and goes
    /// LOW when idle. Instead of polling in a tight loop, we execute
    /// WFI to sleep the CPU between checks. The BUSY pin's falling-
    /// edge GPIO interrupt (armed in Board::init) wakes us immediately
    /// on completion; the 10ms timer interrupt provides a backstop.
    ///
    /// Power savings: ~35mA avoided during each refresh cycle
    /// (600ms partial, 1600ms full) by sleeping instead of spinning.
