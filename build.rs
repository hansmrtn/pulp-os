...
fn generate_bitmap_fonts() {
    ...
    let font_dir = Path::new("src/fonts");
    ...
    if let Some(ref path) = regular {
        ...
        eprintln!(
            "cargo:warning=font: rasterising {} ({} glyphs) at {}px + {}px",
            path.file_name().unwrap().to_string_lossy(),
            font.glyph_count(),
            BODY_PX,
            HEADING_PX,
        );
        emit_font(&mut out, &font, "REGULAR_BODY", BODY_PX);
        emit_font(&mut out, &font, "REGULAR_HEADING", HEADING_PX);
    } else {
        emit_stub(&mut out, "REGULAR_BODY");
    ...
    if let Some(ref path) = bold {
        ...
        eprintln!(
            "cargo:warning=font: rasterising {} at {}px",
            path.file_name().unwrap().to_string_lossy(),
            BODY_PX,
        );
        emit_font(&mut out, &font, "BOLD_BODY", BODY_PX);
    } else {
        emit_stub(&mut out, "BOLD_BODY");
    }
    ...
    if let Some(ref path) = italic {
        ...
        emit_font(&mut out, &font, "ITALIC_BODY", BODY_PX);
    } else {
        emit_stub(&mut out, "ITALIC_BODY");
    }
}
